...
Every subcommand accepts --base-path (default: cwd). Paths are resolved
against it and must stay inside it, so run the tool from anywhere by pointing
--base-path at the project root.
...
class DocReviewer:
    ...
    def update_indexes(
        ...
        `path` is the documentation root to count files under. It must be
        forwarded by callers that accept their own --path, otherwise the doc
        total reported here describes a different tree than the one scanned.
        """
        self.dry_run = dry_run
...
def main():
    ...
    # --base-path is shared by every subcommand. It lives on a parent parser
    # rather than the top-level one so it can be passed after the subcommand
    # name; declaring it in both places would let the subparser default
    # silently overwrite a top-level value.
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument(
        "--base-path",
        default=".",
        help="Project root that paths are resolved against (default: cwd)",
    )

    # scan command
    scan_parser = subparsers.add_parser("scan", help="Scan documentation health", parents=[common])
    ...
    links_parser = subparsers.add_parser("validate-links", help="Validate documentation links", parents=[common])
    ...
    markers_parser = subparsers.add_parser("validate-markers", help="Validate verification markers", parents=[common])
    markers_parser.add_argument("--path", "-p", default="docs/", help="Path to scan")

    # verify command
    verify_parser = subparsers.add_parser("verify", help="Verify doc against source", parents=[common])
    ...
    index_parser = subparsers.add_parser("update-indexes", help="Update navigation indexes", parents=[common])
    index_parser.add_argument("--path", "-p", default="docs/", help="Path to scan for the doc count")
    ...
    full_parser = subparsers.add_parser("full-maintenance", help="Run full maintenance", parents=[common])
    ...
    # Logging is configured here, not at import time: importing this module
    # must not install a root handler on its host process.
    logging.basicConfig(
        level=logging.INFO,
    ...
    reviewer = DocReviewer(args.base_path)
